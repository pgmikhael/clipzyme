...
if __name__ == "__main__":
    ...
    for ecid, ecdict in tqdm(brenda_dataset["data"].items()):
        ...
        for key in ["natural_reaction", "reaction"]:
            if key in ecdict:
                brenda_mols.update( [e for d in ecdict[key] for e in d.get('educts', []) ] )
                brenda_mols.update( [e for d in ecdict[key] for e in d.get('products', [] ) ] )
    ...
    brenda_molecules_dict= {
        d['name']: d for d in brenda_molecules
    }
