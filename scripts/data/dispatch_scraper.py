...
def launch_job(flag_string, filepath, logpath):
    shell_cmd = "python -u scripts/data/bigg_scraper.py {} > {} 2>&1".format(flag_string, logpath)
...
if __name__ == "__main__":
    ...
    for i in range(len(job_list)):
        ...
        try:
            ...
            print("({}/{}) \t SUCCESS: CREATED {}".format(i+1, len(job_list), organism))
            summary_result["status"] = "success"
        except Exception as e:
            print("({}/{}) \t FAILED TO CREATE: {}".format(i+1, len(job_list), organism))
            summary_result["status"] = "failed"
    
        with open(os.path.join(args.save_dir, "dataset_creation_summary.csv"), 'a', newline='') as f_object:  
            dictwriter_object = DictWriter(f_object, fieldnames=["organism", "status", "log_file", "date"])
