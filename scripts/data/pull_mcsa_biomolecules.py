...
def check_fasta_with_residues(sequence, residues):
    ...
    for res_id, res_name in residues:
        if isinstance(res_id, int) and (res_name != ""):
            if (res_id > len(sequence) ) or (sequence[res_id - 1] != RESIDE_TABLE[res_name]):
                return False 
    return True 
...
def get_protein_fasta(uniprot_tuple):
    ...
    return {"uniprot": uniprot, "sequence": sequence, "is_assembly": is_assembly, "parent_uniprot": parent_uniprot} 
