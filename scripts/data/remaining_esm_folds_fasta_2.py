...
def json_to_fasta(args):
    ...
    fasta_file_name = (
        args.save_dir
        + args.json_dir.split("/")[-1].replace(".json", "")
        + "_remaining_"
        + str(counter)
        + ".fasta"
    )
    ...
    prot_ids = set(
        [
            int(
                p.replace(
                    "/Mounts/rbg-storage1/datasets/Enzymes/DLKcat/DeeplearningApproach/Data/database/Kcat_combination_0918_wildtype_mutant_structures/seq_id_",
                    "",
                ).replace(".pdb", "")
            )
            for p in glob.glob(
