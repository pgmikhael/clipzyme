...
class Args(Tap):
    ...
    moa_columns: List[str]  # List of column names to use as MOA columns (e.g., "Target_x", "Target_y", "Pathway").
...
def create_moa_dataset(args: Args) -> None:
    ...
    for moa_column in args.moa_columns:
        data[moa_column].replace({OTHERS: np.nan, OTHERS.lower(): np.nan, '0': np.nan}, inplace=True)
    ...
    include_moas = {
        moa for moa, count in moa_counts.most_common()
        if count >= args.min_mols_per_moa
    }
    smiles_to_moas = {
        smiles: sorted(filtered_moas)
        for smiles, moas in smiles_to_moas.items() if len(filtered_moas := moas & include_moas) > 0
    }

    # Print stats
    print(f'Number of SMILES after removing MOAs with fewer than {args.min_mols_per_moa} '
    ...
    data = pd.DataFrame(data={
        ...
        **{
            moa: [1 if moa in smiles_to_moas[smiles] else 0 for smiles in smiles_list]
            for moa in moa_list
        }
    })
