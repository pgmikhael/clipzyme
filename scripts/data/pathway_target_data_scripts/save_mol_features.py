...
sys.path.append( os.path.join(dirname(dirname(dirname(realpath(__file__)))), 'chemprop_pkg') )
...
def generate_and_save_features(args: Args):
    ...
    if args.from_csv:
        ...
        raise NotImplementedError('SMILES can be loaded from csv or manually for the following datasets [depmap]')
    ...
    if args.restart:
        ...
        if os.path.exists(args.save_path):
            precomputed_smiles = [i.split('.npz')[0] for i in os.listdir(args.save_path) ]

    if not os.path.exists(args.save_path):
        makedirs(args.save_path)

    # Build features map function
    smiles = [s for s in smiles if md5(s) not in precomputed_smiles]  # restrict to data for which features have not been computed yet
    ...
    for i, feats in tqdm(enumerate(features_map), total=len(smiles)):
        save_features(os.path.join(args.save_path, '{}.npz'.format(md5(smiles[i])) ), feats)
