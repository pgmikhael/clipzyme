...
meta = json.load(open("/Mounts/rbg-storage1/datasets/Metabo/datasets/recon3d_dataset.json", "rb"))
...
for sample in tqdm(meta):
    ...
    if 'proteins' in sample:
        for protein in sample['proteins']:
            ...
            if protein["protein_sequence"] is None or len(protein["protein_sequence"]) == 0:
                missing_prots_name.add(protein["bigg_gene_id"])
                TOTAL_MISSING_SEQS += 1

print(f"Total missing SMILES: {TOTAL_MISSING_SMILES} out of {len(metabolites)} metabolites, and total missing sequences: {TOTAL_MISSING_SEQS} out of {len(proteins)} proteins")
