...
class WLDN(AbstractModel):
    def __init__(self, args):
        ...
        if args.use_wln_encoder:
            ...
            self.final_transform = nn.Sequential(
                nn.Linear(args.wln_enc_hidden_dim + 1 if self.add_scores_features else args.wln_enc_hidden_dim, args.wln_enc_hidden_dim),
                nn.ReLU(),
            ...
            self.final_transform = nn.Sequential(
                nn.Linear(args.chemprop_hidden_dim + 1 if args.add_scores_to_edge_attr else args.chemprop_hidden_dim, args.chemprop_hidden_dim),
            ...
            self.final_transform = nn.Sequential(
                nn.Linear(args.gat_hidden_dim + 1 if self.add_scores_features else args.gat_hidden_dim, args.gat_hidden_dim),
                nn.ReLU(),
                nn.Linear(args.gat_hidden_dim, 1)
        )

        self.use_cache = (args.cache_path is not None ) and (not args.load_wln_cache_in_dataset)
        if self.use_cache:
    ...
    def get_product_candidate_list(self, batch, sample_ids):
        ...
        mode = "test" if not self.args.train else "train" # this is used to get candidates, using robust_edit_mol when in predict later for actual smiles generation
        if self.use_cache:
            if not all( self.cache.exists(sid) for sid in sample_ids ):
                with torch.no_grad():
                    ...
                    if getattr(self.args, "mask_suv_scores", False):
                        reactivity_output["s_uv"][~batch['reactants'].mask.bool()] = -torch.inf
...
class ECReactivityCenterNet(ReactivityCenterNet):
    ...
    def forward(self, batch):
        ...
        c_final = self.lin(torch.cat([cs, c_tildes, ec_feats], dim=-1)) # N x 3*hidden_dim -> N x hidden_dim
...
class ECWLDN(WLDN):
    def __init__(self, args):
        ...
        self.final_transform = nn.Sequential(
                nn.Linear(2 * args.wln_enc_hidden_dim + 1 if self.add_scores_features else 2 * args.wln_enc_hidden_dim, args.wln_enc_hidden_dim),
                nn.ReLU(),
                nn.Linear(args.wln_enc_hidden_dim, 1)
            )
    ...
    def forward(self, batch):
        ...
        for idx, product_candidates in enumerate(product_candidates_list):
            ...
            graph_feats = torch.concat([graph_feats, ec_feats[idx].unsqueeze(0).repeat(graph_feats.shape[0], 1) ], dim = -1)
...
class ChempropWLDN(WLDN):
    ...
    def forward(self, batch):
        ...
        for idx, product_candidates in enumerate(product_candidates_list):
            ...
            # add edge vectors
            sum_vectors = dense_candidate_edge_feats + dense_reactant_edge_feats[idx][:num_nodes, :num_nodes].unsqueeze(0) # B, N, N, D

            # get matrix for scores consistent with edge_attr 
            score_features = torch.zeros_like(dense_candidate_edge_feats)[...,:1]
            for cid, cand_changes in enumerate(product_candidates.candidate_bond_change):
            ...
            if self.args.aggregate_over_edges:
                ...
                if self.args.add_scores_to_edge_attr:
                    sum_edge_feats = torch.concat([sum_edge_feats, score_features_edge_attr], dim = -1)
                score_per_edge = self.final_transform(sum_edge_feats)
                score = scatter(score_per_edge, edge_batch, dim = 0, reduce="mean")
            else:
                sum_node_feats, _ = to_dense_batch(sum_node_feats, product_candidates.batch) # num_candidates x max_num_nodes x D
            ...
            if self.args.add_core_score:
                core_scores = [sum(c[-1] for c in cand_changes) for cand_changes in product_candidates.candidate_bond_change]
                core_scores = torch.tensor(core_scores, device = score.device).unsqueeze(-1)
                score = score + torch.log_softmax(core_scores, dim =0) #torch.tensor(core_scores, device = score.device).unsqueeze(-1)
            candidate_scores.append(score) # K x 1
...
class ChempropCGR(WLDN):
    ...
    def forward(self, batch):
        ...
        dense_reactant_node_feats, _ = to_dense_batch(batch["reactants"].x, batch["reactants"].batch) # batch size, max num nodes, feature dimension 

        dense_reactant_edge_feats = to_dense_adj(
            edge_index = batch["reactants"].edge_index, 
            edge_attr = batch['reactants'].edge_attr + 1, # ! add 1 since encoding no edge here
            batch=batch["reactants"].batch
        )
        ...
        for idx, product_candidates in enumerate(product_candidates_list):
            ...
            dense_candidate_edge_feats = to_dense_adj(
                edge_index = product_candidates.edge_index, 
                edge_attr = product_candidates.edge_attr + 1, # ! add 1 since encoding no edge here
                batch=product_candidates.batch
            )

            # node features
            x, _ = to_dense_batch(product_candidates.x, product_candidates.batch) # batch size, num nodes, feature dimension 
            ncands, num_nodes, _ = x.shape
            cgr_nodes = torch.cat([
                dense_reactant_node_feats[idx][:num_nodes].unsqueeze(0).repeat(ncands, 1, 1),
                x], dim = -1)
            product_candidates.x = cgr_nodes.view(-1, cgr_nodes.shape[-1])
            
            # edge features
            cgr_attr = torch.cat([
                dense_reactant_edge_feats[idx][:num_nodes, :num_nodes].unsqueeze(0).repeat(ncands, 1, 1, 1), 
                dense_candidate_edge_feats], dim = -1) # B, N, N, D
...
from transformers import BertConfig, BertModel, AutoTokenizer, EsmModel, BertTokenizer, BertLMHeadModel, BertForMaskedLM
...
class ESMReactivityCenterNet(ReactivityCenterNet):
    ...
    def forward(self, batch):
        ...
        c_final = self.lin(torch.cat([cs, c_tildes, c_protein], dim=-1)) # N x 3*hidden_dim -> N x hidden_dim
        ...
        # predict ecs 
        ec_logits = self.ec_classifier({'x': c_protein_pool})['logit'] if self.predict_ecs else None
...
class ChempropESMCGR(WLDN):
    def __init__(self, args):
        ...
        self.final_transform = nn.Sequential(
                nn.Linear(args.chemprop_hidden_dim * 2 + 1 if args.add_scores_to_edge_attr else args.chemprop_hidden_dim * 2, args.chemprop_hidden_dim),
                nn.LayerNorm(args.chemprop_hidden_dim),
                nn.ReLU(),
                nn.Linear(args.chemprop_hidden_dim, 1)
            )
    ...
    def forward(self, batch):
        ...
        for idx, product_candidates in enumerate(product_candidates_list):
            ...
            sum_node_feats, sum_node_feats_mask = to_dense_batch(sum_node_feats, product_candidates.batch) # num_candidates x max_num_nodes x D
            protein_rxn_feats = protein_feats[idx][:protein_mask[idx].sum()].unsqueeze(0).repeat(sum_node_feats.shape[0], 1, 1)
        ...
        ec_logits = self.ec_classifier({'x': true_candidate_product})['logit'] if self.predict_ecs else None
...
class ChempropESMCGRLate(WLDN):
    ...
    def forward(self, batch):
        ...
        for idx, product_candidates in enumerate(product_candidates_list):
            ...
            sum_node_feats, sum_node_feats_mask = to_dense_batch(sum_node_feats, product_candidates.batch) # num_candidates x max_num_nodes x D
            protein_rxn_feats = protein_feats[idx][1:(protein_mask[idx].sum()-1)].mean(0).unsqueeze(0).repeat(sum_node_feats.shape[0], 1)
...
class TransformerRanker(WLDN):
    def __init__(self, args):
        ...
        if args.mlm_model_path:
            ...
            econfig = self.get_transformer_config(state_dict["hyper_parameters"]["args"])
            self.model =  BertForMaskedLM(econfig) 
            self.model.load_state_dict({k[len("model.model."):]: v for k,v in state_dict["state_dict"].items() if k.startswith("model.model")})
        else:
            self.model =  BertForMaskedLM(econfig)
    ...
    def forward(self, batch):
        reactant_smiles = [ remove_atom_maps(s, sanitize=False) for s in batch["reactants"].smiles ]
        ...
        for idx, product_candidates in enumerate(product_candidates_list):
            # get node features for candidate products
            candidate_smiles = [ remove_atom_maps(s, sanitize=False) for s in product_candidates.smiles ]
            candidate_reactions = ["{}>>{}".format(reactant_smiles[idx], c) for c in candidate_smiles]
            
            tokenized_inputs = self.tokenize(candidate_reactions, self.tokenizer, self.args) 
