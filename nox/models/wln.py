...
class ESMReactivityCenterNet(ReactivityCenterNet):
    ...
    def encode_sequence(self, batch):
        ...
        encoder_hidden_states = encoder_outputs["last_hidden_state"] # ["pooler_output"].unsqueeze(1)
    ...
    def forward(self, batch):
        ...
        # protein
        protein_feats, protein_mask = self.encode_sequence(batch)
        dense_cs, cs_mask = to_dense_batch(cs, batch['reactants'].batch)
        c_protein, _ = self.mha(
            query=dense_cs, 
            key=protein_feats, 
            value=protein_feats, 
            key_padding_mask=protein_mask)
        c_protein = c_protein[cs_mask]
