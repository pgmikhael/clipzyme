...
"""
EGNN Implementation from https://github.com/lucidrains/egnn-pytorch/blob/main/egnn_pytorch/egnn_pytorch.py
"""
...
@register_object("egnn_net", "model")
class EGNNNet(AbstractModel):
    def __init__(self, args):
        ...
        dim = args.dim
        num_adj_degrees = (
            args.num_adj_degrees if hasattr(args, "num_adj_degrees") else None
        )
        global_linear_attn_every = (
            args.global_linear_attn_every
            if hasattr(args, "global_linear_attn_every")
            else 0
        )
        global_linear_attn_heads = (
            args.global_linear_attn_heads
            if hasattr(args, "global_linear_attn_heads")
            else 8
        )
        global_linear_attn_dim_head = (
            args.global_linear_attn_dim_head
            if hasattr(args, "global_linear_attn_dim_head")
            else 64
        )
        num_global_tokens = (
            args.num_global_tokens if hasattr(args, "num_global_tokens") else 4
        )
        assert not (
            exists(num_adj_degrees) and num_adj_degrees < 1
        ), "make sure adjacent degrees is greater than 1"
        self.num_positions = args.num_positions if hasattr(args, "num_positions") else None

        num_tokens = args.num_tokens if hasattr(args, "num_tokens") else None
        self.token_emb = nn.Embedding(num_tokens, dim) if exists(num_tokens) else None
        self.pos_emb = (
            nn.Embedding(num_positions, dim) if exists(num_positions) else None
        )
        num_edge_tokens = (
            args.num_edge_tokens if hasattr(args, "num_edge_tokens") else None
        )
        edge_dim = args.edge_dim if hasattr(args, "edge_dim") else 0
        self.edge_emb = (
        ...
        adj_dim = args.adj_dim if hasattr(args, "adj_dim") else 0
        self.adj_emb = (
    ...
    def forward(self, batch):
        feats = batch['feats']
        coors = batch['coors']
        adj_mat = batch['adj_mat'] if 'adj_mat' in batch else None
        edges = batch['edges'] if 'edges' in batch else None
        mask = batch['mask'] if 'mask' in batch else None
        return_coor_changes = self.args.return_coor_changes
