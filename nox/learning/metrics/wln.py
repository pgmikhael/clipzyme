...
class CandidateTopK(Metric, Nox):
    def __init__(self, args) -> None:
        ...
        for k in args.candidate_topk_values:
            self.add_state('top_{:d}_candidate_changes'.format(k), default=torch.tensor(0), dist_reduce_fx="sum")
    ...
    @property
    def metric_keys(self):
        return ["real_bond_changes", "candidate_probs", "product_candidates_list", "reactant_smiles", "product_smiles", "all_product_smiles"]
    ...
    def update(self, predictions_dict, args):
        ...
        for reaction_idx, reaction_pred in enumerate(candidate_probs):
            ...
            if (args.eval_all_real_bond_changes) and len(all_product_smiles[reaction_idx]) > 1:
                acc = max(args.candidate_topk_values) + 1
                for j, (product_smiles_for_change, bond_change) in enumerate(all_product_smiles[reaction_idx]):
                    product_mol_for_change = Chem.MolFromSmiles(product_smiles_for_change)
                    nonstereo_batch_found_info = examine_topk_candidate_product(args.candidate_topk_values, topk_combos, reactant_mol, bond_change, product_mol_for_change, remove_stereochemistry=True)
                    if any(nonstereo_batch_found_info[f"top_{k}_sanitized"] for k in args.candidate_topk_values):
                        best_rank = min(k for k in args.candidate_topk_values if nonstereo_batch_found_info[f"top_{k}_sanitized"])
                        if best_rank < acc:
                            acc = best_rank
                            product_smiles_for_change, real_bond_changes = all_product_smiles[reaction_idx][j]
            ...
            for k in args.candidate_topk_values:
                if set(real_bond_changes) in [set(c) for c in topk_combos[:k]]:
                    ...
                    setattr(self, f"top_{k}_candidate_changes", current + torch.tensor(1).to(self.device))            
    ...
    def compute(self):
        ...
        for k in self.candidate_topk_values:
            ...
            stats_dict[f"top_{k}_candidate_changes"] = getattr(self, f"top_{k}_candidate_changes").float() / self.total
...
class WLNAccuracy(Metric, Nox):
    def __init__(self, args) -> None:
        ...
        self.add_state("any_correct", default=torch.tensor(0, dtype=torch.float), dist_reduce_fx="sum")
        self.add_state("total", default=torch.tensor(0), dist_reduce_fx="sum")
 
    @property
    def metric_keys(self):
        return ["candidate_probs", "candidate_preds", "candidate_golds", "product_candidates_list"]
