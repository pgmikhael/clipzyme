...
class DigressSimpleVLBLoss(Nox):
    ...
    def __call__(self, model_output, batch, model, args):
        ...
        log_pN = args.dataset_statistics.nodes_dist.log_prob(N)
        ...
        prob0 = self.reconstruction_logp(t, X, E, y, node_mask, model, args)
    ...
    def kl_prior(self, X, E, y, node_mask, model, args):
    ...
    def reconstruction_logp(self, t, X, E, y, node_mask, model, args):
        # Compute noise values for t = 0.
        t_zeros = torch.zeros_like(t)
        beta_0 = model.model.noise_schedule(t_zeros)
        Q0 = model.model.transition_model.get_Qt(beta_t=beta_0, device=model.device)
        ...
        denoiser_input = {"X": noisyX, "E": noisyE, "y": noisyy, "node_mask": node_mask}
        pred0 = model.model.model(denoiser_input)  # graph denoiser
