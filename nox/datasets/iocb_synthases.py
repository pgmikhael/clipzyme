...
class IOCB(AbstractDataset):
    def assign_splits(self, dataset, split_probs, seed=0) -> None:
        ...
        if self.args.split_type == "fold":
            ...
            raise ValueError(
                f"Split {self.args.split_type} type not supported. Must be one of [fold, fixed]"
            )
    ...
    def create_dataset(
        ...
        if self.args.reaction_side == "reactant":
            ...
            smiles_key = (
                ...
                if self.args.use_stereo
                else "SMILES_product_canonical_no_stereo"
            )
        ...
        if self.args.topk_substrates_to_remove is not None:
            substrates = Counter(
                [r for d in self.metadata_json for r in d[smiles_key]]
    ...
    def __getitem__(self, index):
        ...
        try:
            if self.args.use_graph_version:
                ...
                item = {
                    "reaction": "{}>>{}".format(
                        sample["reactants"], sample["products"]
                    ),
                    "reactants": reactants,
                    "products": products,
                    "sequence": sequence,
    ...
    def set_args(args) -> None:
        ...
        args.dataset_file_path = "/Mounts/rbg-storage1/datasets/Enzymes/IOCB/IOCB_TPS-1April2023_verified_non_minor_tps_with_neg.json"
...
class IOCBReactions(IOCB):
    ...
    def post_process(self, args):
        ...
        args.ec_levels = {}
        ecs = [
            ["1", "1", "1", f"{i}"] for i in range(3237)
        ]  # Hardcoded so can use EC prediction model
    ...
    def set_args(args) -> None:
        IOCB.set_args(args)
        args.dataset_file_path = "/Mounts/rbg-storage1/datasets/Enzymes/IOCB/TPS-IOCB-with-mapped-rxn-fixedv1.json"
    ...
    def create_dataset(
        ...
        if self.args.reaction_side == "reactant":
            smiles_key = (
                "substrate_SMILES"
                if self.args.use_stereo
                else "substrate_SMILES_canonical_without_stereochem"
            )
