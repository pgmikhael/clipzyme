...
class Linker(pl.LightningModule, Nox):
    """
    ...
    def validation_step(self, batch, batch_idx, optimizer_idx=None):
        ...
        data = self.trainer.val_dataloaders[0].split_graph
        ...
        filtered_data = getattr(self.trainer.val_dataloaders[0], "filtered_data", data)

        t_mask, h_mask = nbf_utils.strict_negative_mask(filtered_data, batch["triplet"])
        
        pos_h_index, pos_t_index, _ = batch["triplet"].t()

        t_ranking = nbf_utils.compute_ranking(t_output['preds_dict']['logit'], pos_t_index, t_mask)
        h_ranking = nbf_utils.compute_ranking(h_output['preds_dict']['logit'], pos_h_index, h_mask)
    ...
    def test_step(self, batch, batch_idx):
        ...
        * save_hiddens: will save the value of output['preds_dict']['hidden']
        ...
        data = self.trainer.test_dataloaders[0].split_graph
        ...
        filtered_data = getattr(self.trainer.test_dataloaders[0], "filtered_data", data)
